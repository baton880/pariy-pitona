...
def write_log(log_file, path, obj_type):
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    log_file.write(f'{timestamp} removed {obj_type}: {path}\n')
    ...
    for root, _, files in os.walk(trash_folder_path):
        for file_name in files:
            ...
            if is_old(file_path, now_ts, age_thr):
                try:
                    os.remove(file_path)
                    write_log(log_file, file_path, 'file')
                except OSError as err:
                    log_file.write(f'error while deleting file {file_path}: {err}\n')
...
def clean_empty_dirs(trash_folder_path, log_file):
    for root, dirs, _ in os.walk(trash_folder_path, topdown=False):
        for d in dirs:
            ...
            try:
                os.rmdir(dir_path)
                write_log(log_file, dir_path, 'directory')
            except OSError:
                continue
